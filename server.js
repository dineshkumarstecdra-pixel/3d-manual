...
const PORT = Number(process.env.PORT || process.env.UPLOAD_SERVER_PORT || 3001);
...
async function connectMongo() {
  if (!MONGO_URI) {
    throw new Error("MONGO_URI missing. Please add MONGO_URI in .env file.");
  }
  ...
  await mongoDb.collection("vehicles").createIndex({ id: 1 }, { unique: true });
  await mongoDb.collection("orders").createIndex({ id: 1 }, { unique: true });
  await mongoDb.collection("programRevisions").createIndex({ id: 1 }, { unique: true });
...
async function replaceCollection(name, items) {
  const safeItems = Array.isArray(items) ? items.map(removeMongoId) : [];

  await collection(name).deleteMany({});

  if (safeItems.length) {
    await collection(name).insertMany(safeItems);
  }
}
...
async function writeDatabase(vehicles) {
  await replaceCollection("vehicles", vehicles);
}
...
async function writeOrders(orders) {
  await replaceCollection("orders", Array.isArray(orders) ? orders : []);
}

async function readProgramRevisions() {
  return collection("programRevisions")
    .find({}, { projection: { _id: 0 } })
    .sort({ createdAt: -1 })
    .toArray();
}
...
async function writeProgramRevisions(revisions) {
  await replaceCollection("programRevisions", Array.isArray(revisions) ? revisions : []);
}
