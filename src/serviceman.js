...
// 8. OPEN VEHICLE ROUTING (Service Manual opens PDF dict)
window.openVehicle = function(vehicle) {
    localStorage.setItem("selectedVehicle", vehicle); // optional
    
    // PDF Dictionary
    const manuals = {
        bmw_m3: "/manuals/bmw_m3.pdf",
        audi_r8: "/manuals/audi_r8.pdf",
        audi_rs7: "/manuals/audi_rs7.pdf",
        bmw_m4: "/manuals/bmw_m4.pdf"
    };

    // Note: If you upload NEW cars dynamically, they won't be in this hardcoded dict.
    // We handle that gracefully by falling back to a dynamic path if the dict misses it:
    if (manuals[vehicle]) {
        window.open(manuals[vehicle], "_blank");
    } else {
        window.open(`/manuals/${vehicle}.pdf`, "_blank"); // 🔥 Dynamic fallback for new uploads
    }
}
