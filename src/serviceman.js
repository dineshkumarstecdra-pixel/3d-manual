...
function renderVehicles(vehicles) {
  ...
  vehicles.forEach((vehicle) => {
    ...
    card.setAttribute("aria-label", `Open service manual for ${vehicle.name}`);
    card.innerHTML = `
...
function openVehicle(vehicle) {
  ...
  if (vehicle.manualUrl) {
    window.open(vehicle.manualUrl, "_blank");
  } else {
    window.open(`/manuals/${vehicle.id}.pdf`, "_blank");
  }
}
