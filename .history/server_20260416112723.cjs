...
// 1. Serve your HTML files from the root folder
app.use(express.static(__dirname)); 

// 2. NEW: Tell the server to look inside "public" for images, models, and manuals
app.use(express.static(path.join(__dirname, 'public')));

// 3. UPDATED: Tell Multer to save files into your public/ folders
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        if (file.fieldname === 'image') cb(null, './public/images/vehicles/');
