...
function syncTableHighlight() {
  ...
  document.querySelectorAll("#partsList tr").forEach(row => {
    ...
    if (row.dataset.name === selectedPart?.name) {
...
window.addEventListener("click", (event) => {
 ...
 // 🔥 PROFESSIONAL CAD FIX: Only allow the mouse to hit parts that are currently visible
  ...
  if (isIsolated) {
    ...
    if (intersects.length > 0 && intersects[0].object === selectedPart) {
...
isolateOption.onclick = () => {
  ...
  clearSelection()   // 🔥 hide part details panel
...
colorCanvasBtn.onclick = () => {
  colorCanvasBtn.classList.add("active")
  parts.forEach(p => {
    // Rich HSL colors
