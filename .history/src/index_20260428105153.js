  ...
  fetch(excelFilePath)
    ...
    // 4. Excel டேட்டாவை உங்கள் 3D மாடலோடு இணைக்கிறோம்
      jsonData.forEach((row) => {
        // Note: Make sure your Excel file has a column exactly named "Part Name"
        const partKey = row["Part Name"] ? row["Part Name"].trim() : null;
        
        if (partKey) {
          partDescriptions[partKey] = {
            displayName: partKey,
            category: row["System Category"] || "Others", // Keeps your sidebar tree working
            position: row["Position"] || "General",
            // 🔥 NEW: Saves ALL columns from your uploaded Excel file!
            excelData: row 
          };
        }
      });
