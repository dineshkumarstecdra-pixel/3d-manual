...
function selectPart(partOrArray) {
  ...
  const data = partDescriptions[selectedPart.name]

  if (data) {
    document.getElementById("part").innerText = data.displayName
    ...
    floatingLabel.innerHTML = `
      ...
      <div style="font-size:12px; opacity:0.85; word-break: break-word;">
        ${data.description}
      </div>
    `
  } else {
...
/* ================= label ================= */
/* ================= label (TOP-RIGHT EDGE FIX) ================= */
function updateFloatingLabel() {
  if (!floatingLabel) return;
  if (!selectedPart || floatingLabel.style.display !== "block") return;

  // 1. தேர்ந்தெடுக்கப்பட்ட பாகத்தின் எல்லையைக் (Bounding Box) கண்டுபிடிக்கிறோம்
  const box = new THREE.Box3().setFromObject(selectedPart);
  const min = box.min;
  const max = box.max;

  // 2. அந்தப் பெட்டியின் 8 மூலைகளையும் (Corners in 3D Space) எடுக்கிறோம்
  const corners = [
  ...
  // 3. ஒவ்வொரு மூலையையும் 2D ஸ்கிரீனுக்கு மாற்றி, Top-Right புள்ளியைக் கண்டுபிடிக்கிறோம்
  corners.forEach(corner => {
    const projected = corner.clone().project(camera);
    
    // கேமராவுக்குப் பின்னால் இருந்தால் தவிர்க்கிறோம்
    if (projected.z > 1) return;

    // 3D புள்ளியை 2D பிக்சல்களாக மாற்றுதல்
    const screenX = (projected.x * 0.5 + 0.5) * canvasWidth + SIDEBAR_WIDTH;
    const screenY = (-projected.y * 0.5 + 0.5) * canvasHeight;

    // திரையில் வலது ஓரத்திலும் (Max X), மேல் ஓரத்திலும் (Min Y) உள்ள புள்ளியைத் தேடுகிறோம்
    if (screenX > rightmostX) rightmostX = screenX;
    if (screenY < topmostY) topmostY = screenY; // திரையில் Y மதிப்பு மேலிருந்து கீழாக அதிகரிக்கும்
  });

  // 4. லேபிளை அந்தப் புள்ளியில் வைக்கிறோம் (மாடலை உரசாமல் இருக்க 15px தள்ளி வைக்கிறோம்)
  const labelWidth = floatingLabel.offsetWidth || 260;
  const labelHeight = floatingLabel.offsetHeight || 100;

  let finalX = rightmostX + 15; 
  let finalY = topmostY - 15;   

  // 5. லேபிள் திரையை விட்டு வெளியே (Screen Cutoff) போகாமல் தடுக்க
  if (finalX + labelWidth > window.innerWidth) {
    finalX = rightmostX - labelWidth - 15; // இடம் இல்லையென்றால் இடதுபுறம் திருப்பிக் காட்டுகிறோம்
  }
  if (finalY < 0) {
    finalY = 15; // மேலே இடித்துக்கொண்டால் கொஞ்சம் கீழே இறக்குகிறோம்
  }

  // பழைய Center அலைன்மெண்ட்டை நீக்கிவிட்டு புதிய இடத்தைச் செட் செய்கிறோம்
