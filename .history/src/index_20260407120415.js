...
function selectPart(partOrArray) {
  ...
  if (Array.isArray(partOrArray)) {
    ...
    selectedPart = partOrArray[0] // UI தேவைக்காக முதல் பார்ட்டை மெயின் ஆக வைக்கிறோம்
  } else {
    selectedPartsGroup = [partOrArray]
  ...
  if (data) {
    ...
    document.getElementById("partDescription").innerText =
      ...
      "\nQuantity: " + selectedPartsGroup.length + // QTY-ஐ சரியாகக் காட்டுகிறோம்
      "\n\n" + data.description
...
function clearSelection() {
  ...
  if (floatingLabel) {
    ...
    setTimeout(() => floatingLabel.style.display = "none", 200)
  }
}
