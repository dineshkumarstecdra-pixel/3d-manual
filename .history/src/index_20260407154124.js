...
loader.load(`/models/${selectedVehicle}.glb`, (gltf) => {
 ...
 // 1. மாடலை Frame செய்கிறோம்
  ...
  // 2. 🔥 ஆட்டோமேட்டிக்காக CSV ஃபைலை பின்னணியில் லோட் செய்கிறோம்
  Papa.parse("./parts_data.csv", {
    download: true, // ஃபைலை நாமே டவுன்லோட் செய்து படிக்கச் சொல்கிறோம்
    header: true,   // முதல் வரியை தலைப்புகளாக (Headers) எடுத்துக்கொள்ளும்
    complete: function(results) {
      
      const jsonData = results.data;
      partDescriptions = {};

      // 3. CSV டேட்டாவை உங்கள் 3D மாடலோடு இணைக்கிறோம்
      jsonData.forEach((row, index) => {
        const partKey = row["Part Name"]; 
        
        if (partKey) {
          partDescriptions[partKey] = {
            sno: row["SI.NO"] || (index + 1),
            displayName: partKey,
            category: row["System Category"] || "Others",
            position: "General",
            description: row["Technical Description"] || "No description provided."
          };
        }
      });

      console.log("CSV Auto-Loaded Successfully! ✅", partDescriptions);

      // 4. டேட்டா கிடைத்தவுடன் Sidebar மற்றும் Table-ஐ உருவாக்குகிறோம்
      buildSystemTree();  
      createPartsTable(); 
    },
    error: function(error) {
      console.error("CSV ஃபைலைப் படிப்பதில் பிழை ❌:", error);
      alert("பாகங்களின் டேட்டாவை லோட் செய்ய முடியவில்லை. 'parts_data.csv' ஃபைல் சரியான இடத்தில் உள்ளதா என சரிபார்க்கவும்.");
    }
  });
}, undefined, (error) => {
