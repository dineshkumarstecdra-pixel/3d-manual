...
function renderSystems(systems){
  ...
  container.className = "systems-grid"
  ...
  Object.keys(systems).forEach(system => {
    ...
    btn.innerText = system

  btn.onclick = (e) => {

  const isActive = activeSystems.has(system)

  // 🔥 NORMAL CLICK (no CTRL)
  if (!e.ctrlKey) {

    // 👉 If clicking same active system → turn OFF everything
    if (isActive && activeSystems.size === 1) {
      activeSystems.clear()

      document.querySelectorAll(".system-card").forEach(b => {
  ...
  activeSystems.forEach(sys => {
    selectedParts.push(...systems[sys])
  })
  ...
  if (selectedParts.length > 0) {
    filterPartsTable(selectedParts)
  } else {
    document.querySelectorAll("#partsList tbody tr").forEach(row => {
      row.style.display = ""
    })
