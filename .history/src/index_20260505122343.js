...
// "Select a part..." என்ற டெக்ஸ்ட்டை அப்டேட் செய்யும் வரியை அப்படியே வைக்கலாம் அல்லது நீக்கலாம்

  // 🔥 புதிதாக சேர்க்க வேண்டியது: Selection-ஐ கிளியர் செய்ததும் Popup-ஐ மறைக்க
    ...
    Object.keys(obj).forEach(key => {
      ...
      if (Array.isArray(obj[key])) {
        ...
        obj[key].forEach(partName => {
          ...
          // 🔥 1. Part ID மற்றும் Qty-ஐ கணக்கிடுதல்
          const pData = Object.values(partDescriptions).find(p => p.displayName === partName)
          const partId = pData && pData.partId ? pData.partId : "-"
          
          const matchedParts = parts.filter(p => formatName(p.name) === partName)
          const qty = matchedParts.length
          
          // 🔥 2. UI-ல் அழகாக காண்பிக்க Text-ஐ செட் செய்தல்
          partNode.innerText = `${partId} - ${partName} - Qty: ${qty}`
          
          // 🔥 3. லாஜிக்கிற்காக ஒரிஜினல் பெயரை பின்புலத்தில் மறைத்து வைத்தல்
          partNode.dataset.partName = partName 

          partNode.onclick = (e) => {
            e.stopPropagation()
            // dataset-ல் உள்ள பெயரை வைத்து 3D model-ஐ செலக்ட் செய்தல்
            if (matchedParts.length > 0) {
              selectPart(matchedParts)
            }
          }
...
function syncTreeHighlight(){
  ...
  const selectedName = formatName(selectedPart.name)

  // 1. பழைய Highlights-ஐ அழித்தல்
  document.querySelectorAll(".tree-leaf").forEach(el=>{
    el.classList.remove("active-tree")
  })

  // 🔥 2. புதிதாக சேர்க்கப்பட்டது: திறந்திருக்கும் அனைத்து ஃபோல்டர்களையும் மூடுதல் (Collapse All)
  document.querySelectorAll(".tree-node").forEach(node => {
    // ஐகானை 'கீழ்நோக்கிய அம்புக்குறி'யிலிருந்து 'பக்கவாட்டு அம்புக்குறி'யாக மாற்றுதல்
    node.innerHTML = node.innerHTML.replace("▼", "▶")
    
    // அந்த ஃபோல்டருக்குள் உள்ள child container-ஐ மறைத்தல்
    if (node.nextElementSibling) {
      node.nextElementSibling.style.display = "none"
    }
  })

  // 3. புதிய Part-ஐக் கண்டுபிடித்து Highlight செய்து Expand செய்தல்
  document.querySelectorAll(".tree-leaf").forEach(leaf=>{
    
    // முந்தைய மாற்றத்தின்படி dataset.partName-ஐ ஒப்பிடுகிறோம்
    if(leaf.dataset.partName === selectedName){

      // 🔥 highlight
      leaf.classList.add("active-tree")

      // 🔥 expand parents (இது தேவையானதை மட்டும் திறக்கும்)
      expandParents(leaf)
