...
function renderSystems(systems){
  ...
  Object.keys(systems).forEach(system=>{
    ...
    systemDiv.onclick = ()=>{
      ...
      // 1. Toggle ONLY the clicked folder (Do NOT close the others!)
      if(isOpen){
      ...
      // 2. Gather every part from EVERY currently open folder
      ...
      // 3. Update the 3D Model and the Table with the combined list
      highlightParts(allActiveParts)
      
      if (allActiveParts.length > 0) {
        filterPartsTable(allActiveParts)
      } else {
        // 4. If all folders are closed, reset the table back to normal
