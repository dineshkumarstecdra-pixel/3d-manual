...
loader.load(`/models/${selectedVehicle}.glb`, (gltf) => {
 ...
 gltf.scene.traverse(obj => {
    if (obj.isMesh) {
      ...
      // 🔥 THE FIX: மெமரியை மிச்சப்படுத்த ஆரம்பத்திலேயே Clone செய்வதை நிறுத்திவிட்டோம்.
      parts.push(obj)
...
// 🔥 SMART CONTEXT MENU (EDGE FIX) 🔥
window.addEventListener("contextmenu", (event) => {
  event.preventDefault();

  if (!selectedPart) return;

  // 1. முதலில் மெனுவைக் காட்டுகிறோம் (அப்போதுதான் அதன் உண்மையான அகலம்/உயரத்தை எடுக்க முடியும்)
  contextMenu.style.display = "block";

  // 2. மெனுவின் அளவுகளை எடுக்கிறோம்
  const menuWidth = contextMenu.offsetWidth || 150; // மெனுவின் தோராயமான அகலம்
  const menuHeight = contextMenu.offsetHeight || 100; // மெனுவின் தோராயமான உயரம்

  // 3. மவுஸ் கிளிக் செய்த இடத்தை எடுக்கிறோம்
  let x = event.clientX;
  let y = event.clientY;

  // 4. திரையின் எல்லைகளைத் தாண்டுகிறதா என செக் செய்கிறோம்
  // வலது ஓரத்தில் இடம் இல்லை என்றால், மெனுவை இடதுபுறமாக நகர்த்துகிறோம்
  if (x + menuWidth > window.innerWidth) {
    x = window.innerWidth - menuWidth - 5; // 5px இடைவெளி
  }

  // கீழ் ஓரத்தில் இடம் இல்லை என்றால், மெனுவை மேற்புறமாக நகர்த்துகிறோம்
  if (y + menuHeight > window.innerHeight) {
    y = window.innerHeight - menuHeight - 5;
  }

  // 5. சரியான இடத்தை செட் செய்கிறோம்
