...
function buildSystemTree(){
  ...
  Object.values(partDescriptions).forEach(part=>{
    ...
    const system = part.category || "Others"
    const subsystem = part.subsystem || "General" // 🔥 NEW

    if(!systems[system]) systems[system] = {}

    if(!systems[system][subsystem]){
...
function renderTree(systems){
  ...
  Object.keys(systems).forEach(system => {
    ...
    // TOGGLE OPEN/CLOSE
    systemDiv.onclick = () => {
      subsContainer.classList.toggle("open")
      systemDiv.classList.toggle("active")
    }
    ...
    Object.keys(systems[system]).forEach(sub => {
      ...
      // CLICK → highlight parts
      subDiv.onclick = (e) => {
        e.stopPropagation()

        const parts = systems[system][sub]
        highlightParts(parts)
        filterPartsTable(parts)
      }
