...
function renderOEMTree(tree){
  ...
  let activeNode = null  // track which node is currently selected
    ...
    Object.keys(obj).forEach(key => {
      ...
      node.onclick = (e) => {
        ...
        if (isOpen) {
          ...
          activeNode = null
          // Show all parts
          document.querySelectorAll("#partsList tbody tr").forEach(r => r.style.display = "")
          return
        }

        // OPEN: collapse siblings first
        if (activeNode) {
          activeNode.childrenContainer.style.display = "none"
          activeNode.node.innerHTML = `▶ ${activeNode.key}`
        }

        childrenContainer.style.display = "block"
        node.innerHTML = `▼ ${key}`
        activeNode = { node, childrenContainer, key }

        // Collect all leaf parts under this node
        const allParts = []
        function collectParts(o){
          if (Array.isArray(o)) { allParts.push(...o); return }
          Object.values(o).forEach(v => collectParts(v))
        }
        collectParts(obj[key])

        // Filter parts list to show only matching parts
        filterPartsTable(allParts)
      }
