 ...
 // 1. மாடலை Frame செய்கிறோம்
  ...
  fetch(excelFilePath)
    ...
    .then(buffer => {
      ...
      // 🔥 RAW data-va eduthu headers-a clean pandrom
      const rawJson = XLSX.utils.sheet_to_json(worksheet);
      partDescriptions = {};

      rawJson.forEach((row) => {
        // Inga ovvoru row-laiyum irukkura keys (column names)-a clean pandrom
        const cleanRow = {};
        Object.keys(row).forEach(key => {
          cleanRow[key.trim()] = row[key]; // Headers-la irukkura extra spaces-a remove pannum
        });

        // "Part Name" column irukura row-vukku mattum data-va save pandrom
        const partKey = cleanRow["Part Name"] ? cleanRow["Part Name"].toString().trim() : null;
        
        if (partKey) {
          partDescriptions[partKey] = {
            displayName: partKey,
            category: cleanRow["System Category"] || "Others", 
            excelData: cleanRow // Clean panna row-va apdiye save pandrom
          };
        }
      });

      console.log("Excel Cleaned & Loaded! ✅", partDescriptions);
...
function renderSystems(systems){
  ...
  Object.keys(systems).forEach(system=>{
    ...
    Object.keys(systems[system]).forEach(sub=>{
      ...
      subDiv.onmouseleave = () => {
        // Only clear the hover glow if we aren't in isolation mode
        if (!isIsolated) {
          parts.forEach(p=>{
            if(p.material && p.material.emissive) p.material.emissive.set(0x000000)
          })
  ...
  // 🔥 NEW MULTI-SELECT CLICK LOGIC WITH SHIFT-KEY SUPPORT
    systemDiv.onclick = (e) => { // Make sure to add 'e' here to capture the event!
      ...
      document.querySelectorAll(".system-item.active").forEach(activeDiv => {
        ...
        if(systems[sysName]){
          Object.values(systems[sysName]).forEach(arr=>{
            arr.forEach(p => allActiveParts.push(p))
          })
        }
      })
