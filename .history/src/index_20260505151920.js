...
document.getElementById("partDescription").innerText = "Select a part to see details."
  ...
  // 🔥 BUG FIX: Search Box மற்றும் மறைக்கப்பட்ட Tree பெயர்களை Reset செய்தல்
  const searchBox = document.getElementById("partSearch");
  if (searchBox) searchBox.value = ""; // Search Box-ல் உள்ள நம்பரை அழிக்க

  document.querySelectorAll(".tree-leaf").forEach(leaf => {
    leaf.style.display = ""; // மறைக்கப்பட்ட மற்ற பெயர்களை மீண்டும் கொண்டுவர
    leaf.classList.remove("search-hit"); // Search Highlight-ஐ நீக்க
  });
